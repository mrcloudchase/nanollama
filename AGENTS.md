...
# Options
python nanollama.py --prompt "text" --model MODEL_ID --device cpu --temp 0.7 --max-tokens 200
```
...
- **RoPE** (`precompute_rope` + `apply_rope`) — Rotary Positional Embeddings. Precomputes cos/sin tables indexed by position. `apply_rope` reshapes Q/K into dimension pairs and rotates each pair by position-dependent angles. Only applied to Q and K (not V). Makes attention scores depend on relative position.
...
- **generate()** — Two-phase generation loop:
  ...
  - *Decode*: Generate tokens one at a time. Each step feeds only the new token (start_pos increments), KV-cache provides context. Samples via temperature scaling + multinomial (or argmax when temp=0). Streams each token to stdout.
...
- **No chat template.** The model defaults to TinyLlama-Chat but prompts are passed raw. Chat-tuned models produce gibberish without proper `<|user|>`/`<|assistant|>` formatting — this is expected and documented. Chat templates are a ROADMAP item.
- **Only temperature sampling.** No top-k, top-p, or repetition penalty yet. Those are ROADMAP Phase 1 items.
